        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          ...
          <div className="lg:col-span-7 space-y-6 text-left">
            <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-slate-900/90 border border-slate-700/70 text-slate-300 text-xs font-medium backdrop-blur-sm">
              ...
              <span>{hero?.badge || 'AJT77 • Institutional Price Action & Risk Architecture'}</span>
            </div>

            <div className="space-y-3">
              <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold text-white tracking-tight leading-[1.08]">
                {hero?.titlePrimary || 'Master Markets Through'}{' '}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 via-indigo-300 to-cyan-300">
                  {hero?.titleGradient || 'Pure Price Action.'}
                </span>
              </h1>
              <p className="text-lg sm:text-xl font-medium text-slate-300">
                {hero?.mentorLine || (
                  <>
                    {content.founderName || 'Ajay Gadhe'} <span className="text-slate-600 px-1">|</span> Trader & Mentor <span className="text-slate-600 px-1">|</span> <span className="text-sky-400 font-semibold">{content.subBrand || 'AjayTrades77'}</span>
                  </>
                )}
              </p>
            </div>
            ...
            <p className="text-base sm:text-lg text-slate-400 max-w-2xl leading-relaxed">
              {hero?.description || 'Systematic market understanding built on raw orderflow, liquidity sweep mechanics, and mathematically disciplined risk control. No retail indicator noise, no artificial promises—just structured, transparent execution education.'}
            </p>
            ...
            <div className="flex flex-wrap items-center gap-3.5 pt-2">
              ...
              >
                <span>{hero?.ctaPrimaryText || 'Explore Official Courses'}</span>
              ...
              >
                ...
                <span>{hero?.ctaSecondaryText || 'Trading Journey'}</span>
              </button>
              ...
              >
                ...
                <span>{hero?.ctaTertiaryText || 'Trading Master (Book)'}</span>
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-6 border-t border-slate-800/80">
              <div className="bg-slate-900/70 border border-slate-800/80 rounded-xl p-3.5">
                ...
                <p className="text-sm font-bold text-white mt-1">{hero?.statMethodology || 'Raw Price Action'}</p>
              </div>
              <div className="bg-slate-900/70 border border-slate-800/80 rounded-xl p-3.5">
                <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">AJT77 Basic</p>
                <p className="text-sm font-bold text-sky-400 mt-1">{hero?.statBasicPriceLabel || '₹6,000 INR'}</p>
              </div>
              <div className="bg-slate-900/70 border border-slate-800/80 rounded-xl p-3.5">
                <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">AJT77 Pro</p>
                <p className="text-sm font-bold text-indigo-400 mt-1">{hero?.statProPriceLabel || '₹10,000 INR'}</p>
              </div>
              <div className="bg-slate-900/70 border border-slate-800/80 rounded-xl p-3.5">
                <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Verification</p>
                <p className="text-sm font-bold text-emerald-400 mt-1">{hero?.statVerificationLabel || 'Razorpay Live'}</p>
              </div>
            </div>
          </div>
            ...
            <div className="relative rounded-2xl bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-800 p-6 sm:p-8 shadow-2xl shadow-slate-950/80">
              <div className="flex items-center justify-between pb-4 border-b border-slate-800">
                <div className="flex items-center space-x-2">
                  ...
                  <h3 className="text-base font-bold text-white">{content.brandName || 'AJT77'} Core Principles</h3>
                </div>
              ...
              <div className="mt-6 pt-5 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
                ...
                <span className="text-slate-300 font-semibold">{content.founderName || 'Ajay Gadhe'} • {content.brandName || 'AJT77'}</span>
              </div>
            </div>
          </div>
        </div>
